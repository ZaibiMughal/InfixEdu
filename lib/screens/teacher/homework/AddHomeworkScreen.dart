...
class _AddHomeworkScrrenState extends State<AddHomeworkScrren> {
  ...
  List<TeacherSubject> removeDuplicateValues(List<TeacherSubject> subjectList){
    List<TeacherSubject> temp = [];
    subjectList.forEach((element) {

      print(element.subjectId);
      if ((temp.firstWhere((it) => it.subjectId == element.subjectId, orElse: () => null)) != null) {
  ...
  Widget getSubjectDropdown(List<TeacherSubject> subjectList) {
    // subjectList = removeDuplicateValues(subjectList);
    // print("AFter removing $subjectList");
    return Container(
      width: MediaQuery.of(context).size.width,
      padding: const EdgeInsets.symmetric(horizontal: 10.0, vertical: 10),
      child: DropdownButton(
  ...
  int getSubjectId<T>(List<TeacherSubject> t, String subject) {
    int code;
    for (var s in t) {
